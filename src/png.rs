...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        if bytes.len() < 8 {
            return Err("Data is too short to be a valid PNG".into());
        }

        let header = bytes[..8].try_into()?;
        ...
        while remaining.len() >= 12 {
            ...
            let chunk_type = chunk_type::ChunkType::try_from([
                remaining[4],
                remaining[5],
                remaining[6],
                remaining[7],
            ])?;
            let data = remaining[8..(8 + length as usize)].to_vec();
            let chunk = Chunk::new(chunk_type, data);
            if !chunk.is_valid() {
                return Err("Invalid chunk".into());
            }
            chunks.push(chunk);
            remaining = &remaining[(12 + length as usize)..];
        }
